...
#define CARD_T1_DEFAULT CARD_OMAP4_ABE
...
struct t1_audio_device {
    ...
    int in_call;
    float voice_volume;
    struct t1_stream_in *active_input;
    struct t1_stream_out *outputs[OUTPUT_TOTAL];
    bool mic_mute;
    int tty_mode;
...
static void select_output_device(struct t1_audio_device *adev);
static void select_input_device(struct t1_audio_device *adev);
static int adev_set_voice_volume(struct audio_hw_device *dev, float volume);
static int do_input_standby(struct t1_stream_in *in);
static int do_output_standby(struct t1_stream_out *out);
static void in_update_aux_channels(struct t1_stream_in *in, effect_handle_t effect);
...
static int start_call(struct t1_audio_device *adev)
{
...
static void end_call(struct t1_audio_device *adev)
{
...
static void set_eq_filter(struct t1_audio_device *adev)
{
...
{
    struct t1_audio_device *adev = (struct t1_audio_device *)data;
...
static void set_incall_device(struct t1_audio_device *adev)
{
    int device_type;
...
static void set_input_volumes(struct t1_audio_device *adev, int main_mic_on,
...
static void set_output_volumes(struct t1_audio_device *adev, bool tty_volume)
{
    unsigned int channel;
    int speaker_volume;
    int headset_volume;
...
static void force_all_standby(struct t1_audio_device *adev)
{
    struct t1_stream_in *in;
    struct t1_stream_out *out;
...
static void select_mode(struct t1_audio_device *adev)
{
    if (adev->mode == AUDIO_MODE_IN_CALL) {
...
static void select_output_device(struct t1_audio_device *adev)
{
    ...
    if (adev->mode == AUDIO_MODE_IN_CALL) {
        if (bt_on)
            ...
            /* FIXME: enable sidetone mixer capture if needed */
...
static int start_output_stream_low_latency(struct t1_stream_out *out)
{
    struct t1_audio_device *adev = out->dev;
#ifdef PLAYBACK_MMAP
    ...
    if (adev->out_device & ~(AUDIO_DEVICE_OUT_DGTL_DOCK_HEADSET | AUDIO_DEVICE_OUT_AUX_DIGITAL)) {
        ...
        out->pcm[PCM_NORMAL] = pcm_open(CARD_T1_DEFAULT, PORT_TONES,
    ...
    if (adev->out_device & AUDIO_DEVICE_OUT_DGTL_DOCK_HEADSET) {
        ...
        out->pcm[PCM_SPDIF] = pcm_open(CARD_T1_DEFAULT, PORT_SPDIF,
...
/* must be called with hw device and output stream mutexes locked */
static int start_output_stream_deep_buffer(struct t1_stream_out *out)
{
    struct t1_audio_device *adev = out->dev;
    ...
    out->config[PCM_NORMAL].rate = MM_FULL_POWER_SAMPLING_RATE;
    out->pcm[PCM_NORMAL] = pcm_open(CARD_T1_DEFAULT, PORT_MM,
...
static int start_output_stream_hdmi(struct t1_stream_out *out)
{
    struct t1_audio_device *adev = out->dev;
    ...
    if (adev->outputs[OUTPUT_LOW_LATENCY] != NULL &&
            !adev->outputs[OUTPUT_LOW_LATENCY]->standby) {
        struct t1_stream_out *ll_out = adev->outputs[OUTPUT_LOW_LATENCY];
...
static void add_echo_reference(struct t1_stream_out *out,
...
static void remove_echo_reference(struct t1_stream_out *out,
                                  struct echo_reference_itfe *reference)
{
    pthread_mutex_lock(&out->lock);
...
static void put_echo_reference(struct t1_audio_device *adev,
                          struct echo_reference_itfe *reference)
{
    if (adev->echo_reference != NULL &&
...
static struct echo_reference_itfe *get_echo_reference(struct t1_audio_device *adev,
...
static int get_playback_delay(struct t1_stream_out *out,
                       size_t frames,
...
static int start_input_stream(struct t1_stream_in *in)
{
    int ret = 0;
    struct t1_audio_device *adev = in->dev;
...
{
    struct t1_stream_in *in = (struct t1_stream_in *)stream;
...
static void get_capture_delay(struct t1_stream_in *in,
                       size_t frames,
                       struct echo_reference_buffer *buffer)
{
...
static int32_t update_echo_reference(struct t1_stream_in *in, size_t frames)
{
    struct echo_reference_buffer b;
    b.delay_ns = 0;
...
static void push_echo_reference(struct t1_stream_in *in, size_t frames)
{
                                   ...
                                   struct resampler_buffer* buffer)
{
    ...
    in = (struct t1_stream_in *)((char *)buffer_provider -
                                   offsetof(struct t1_stream_in, buf_provider));
...
static ssize_t read_frames(struct t1_stream_in *in, void *buffer, ssize_t frames)
{
    ssize_t frames_wr = 0;
...
static ssize_t process_frames(struct t1_stream_in *in, void* buffer, ssize_t frames)
{
    ssize_t frames_wr = 0;
...
static int in_configure_reverse(struct t1_stream_in *in)
{
    int32_t cmd_status;
    uint32_t size = sizeof(int);
...
static void in_read_audio_effect_channel_configs(struct t1_stream_in *in,
...
static uint32_t in_get_aux_channels(struct t1_stream_in *in)
{
    int i;
...
static int in_reconfigure_channels(struct t1_stream_in *in,
...
static void in_update_aux_channels(struct t1_stream_in *in,
                                   effect_handle_t effect)
{
    uint32_t aux_channels;
    channel_config_t channel_config;
...
static int out_read_hdmi_channel_masks(struct t1_stream_out *out) {
    int max_channels = 0;
...
{
    struct t1_audio_device *ladev = (struct t1_audio_device *)dev;
    struct t1_stream_out *out;
    int ret;
    int output_type;
    ...
    *stream_out = NULL;
    ...
    out = (struct t1_stream_out *)calloc(1, sizeof(struct t1_stream_out));
    if (!out)
        return -ENOMEM;
...
{
    struct t1_audio_device *adev = (struct t1_audio_device *)dev;
    struct str_parms *parms;
    char *str;
    char value[32];
    int ret;
                                  ...
                                  struct audio_stream_in **stream_in)
{
    ...
    *stream_in = NULL;
    ...
    in = (struct t1_stream_in *)calloc(1, sizeof(struct t1_stream_in));
    if (!in)
        return -ENOMEM;
...
{
    struct t1_audio_device *adev = (struct t1_audio_device *)device;
...
static int adev_open(const hw_module_t* module, const char* name,
                     hw_device_t** device)
{
    struct t1_audio_device *adev;
    int ret;

    if (strcmp(name, AUDIO_HARDWARE_INTERFACE) != 0)
        return -EINVAL;

    adev = calloc(1, sizeof(struct t1_audio_device));
    if (!adev)
        return -ENOMEM;
